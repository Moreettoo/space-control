...
export default function EditMissionScreen() {
  ...
  // Encontra a missão pelo id no array do context.
  const mission = missions.find((m) => m.id === id);

  const [form, setForm] = useState<MissionForm>(
    mission ? { ...mission } : EMPTY_FORM,
  );
  const [errors, setErrors] = useState<FieldErrors>({});
  const [touched, setTouched] = useState<FieldTouched>({});
  const [saving, setSaving] = useState(false);
  const [savedAt, setSavedAt] = useState<string | null>(mission?.savedAt ?? null);
  const [banner, setBanner] = useState<{ name: string; code: string; savedAt: string } | null>(null);

  // Cobre o caso em que o componente monta antes do context terminar de carregar.
  useEffect(() => {
    if (mission && form.name === '') {
      setForm({ ...mission });
      setSavedAt(mission.savedAt);
    }
  }, [mission]);

  // Redireciona se o id não existir após o carregamento do context.
  useEffect(() => {
  ...
  const setSectionLayout = (sid: SectionId) => (event: LayoutChangeEvent) => {
    sectionY.current[sid] = event.nativeEvent.layout.y;
  };
  ...
  const handleSubmit = async () => {
    ...
    setSaving(true);
    try {
      const record = await updateMission(id, form);
      setSavedAt(record.savedAt);
      setBanner({ name: form.name.trim(), code: form.code.trim(), savedAt: record.savedAt });
      scrollRef.current?.scrollTo({ y: 0, animated: true });
    } catch {
      Alert.alert('Falha ao salvar', 'Não foi possível atualizar a missão. Tente novamente.');
    } finally {
      setSaving(false);
    }
  };

  // "Descartar alterações" — restaura os dados originais da missão.
  const handleClear = () => {
    if (mission) {
      setForm({ ...mission });
    }
        ...
        >
          ...
          >
            <View style={styles.column}>
              <Appear>
                <Header editing={true} savedAt={savedAt} />
              </Appear>
              ...
              <Appear delay={240}>
                <View style={styles.actions}>
                  <PrimaryButton
                    label="Atualizar missão"
                    loadingLabel="Salvando…"
                    loading={saving}
                    onPress={handleSubmit}
                  />
                  <GhostButton label="Descartar alterações" onPress={handleClear} />
                  <GhostButton label="← Voltar para lista" onPress={() => router.back()} />
                </View>
              </Appear>
            </View>
          </ScrollView>
...
const styles = StyleSheet.create({
  ...
  rowItem: { flexGrow: 1, flexBasis: 150, minWidth: 0 },
